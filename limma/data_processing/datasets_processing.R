...
transform_to_log2 <- function(rnaseq_tpm_data) {
  if (any(rnaseq_tpm_data < 0, na.rm = TRUE)) {
    warning("Input data contains negative values. Log transformation may not be appropriate.")
  }
...
create_plots_for_expresion_data <- function(rnaseq_tpm_data, log_data, log_data_filtered, file_path) {  

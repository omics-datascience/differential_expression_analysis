...
tiempo_limma <- system.time({
  
    group <- factor(make.names(metadata[[clinical_attribute]]))
  ...
  message("TIEMPO_LINEA_DIF_EXP:", tiempo_limma["elapsed"], "\n")
  message("Análisis diferencial completado. Se generaron ", length(results), " contrastes.")
  return(results)
}
